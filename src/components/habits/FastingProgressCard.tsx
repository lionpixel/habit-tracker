    ...
    >
      ...
      <div className="p-5 relative">
        ...
        <div className="flex items-start justify-between mb-5">
          ...
          {!hasStarted ? (
            <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-white/[0.06] border border-white/[0.1]">
              <Clock size={12} className="text-slate-400" />
              <span className="text-[11px] font-bold text-slate-400">Não iniciado</span>
            </div>
          ) : isComplete ? (
        ...
        {!hasStarted ? (
          <div className="flex flex-col items-center gap-4 py-4">
            <div className="text-center">
              <p className="text-slate-400 text-sm mb-1">Pronto para começar o desafio?</p>
              <p className="text-slate-600 text-xs">{totalDays} dias de disciplina — começa hoje</p>
            </div>
            <motion.button
              whileTap={{ scale: 0.95 }}
          ...
          <>
            ...
            <div className="flex items-baseline gap-2 mb-4">
              ...
              >
                <AnimatedCounter value={progressDays} />
              </span>
              <div>
                <div className="text-slate-300 font-bold text-sm">de {totalDays} dias</div>
                <div className="text-slate-500 text-xs">
                  {daysLeft > 0 ? `${daysLeft} restantes` : 'desafio concluído'}
                </div>
              </div>
            </div>
            ...
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[10px] text-slate-600 uppercase tracking-wider font-semibold">Progresso automático</span>
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              {[
                ...
                },
                {
                  ...
                  value: `${Math.max(progressDays, habit.longestStreak ?? 0)} dias`,
                },
            ...
            >
              ...
              <p className="text-xs text-slate-400">
                Você já completou{' '}
                <span className="font-black tabular-nums" style={{ color }}>{yearTotal}</span>
                {' '}dias de disciplina esse ano
              </p>
            </div>

            {/* Countdown */}
            {daysUntilEnd !== null && daysUntilEnd > 0 && !isComplete && (
              <div
        ...
        {onEdit && (
          <button
            onClick={onEdit}
            className="w-full mt-1 py-2 rounded-xl bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.07] text-slate-400 hover:text-slate-200 text-xs font-semibold transition-all"
          >
            Configurar jejum
          </button>
        )}
